...
#include <pthread.h>
#include "helpers.c"
#include "job_scheduler.c"

#define LINELEN 1000 /* maximum input  */
#define JOBSLEN 1000 /* maximum JOBS  */
#define JOBQLEN 100  /* maximum JOBQ */

int CONCUR;      
int NWORKING;      
job JOBS[JOBSLEN]; /* permanent array  */
queue *JOBQ;       
...
void handle_input()
{
    ...
    printf("Enter `submit COMMAND [ARGS]` to create and run a job.\n"
           "Enter `showjobs` list JOBS that  "
           "currently waiting or working.\n"
           "Enter `submithistory` list JOBS that were "
    ...
    else if (pid > 0) /* parent process */
    {
     //   waitpid(pid, &jp->estat, WUNTRACED);
        ...
        if (!WIFEXITED(jp->estat))
            fprintf(stderr, "Child processes %d did not terminate normally!\n", pid);
    }
    else
    {
